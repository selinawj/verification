...
posterPlot = ggplot(posterTable, aes(x = reorder(poster, -count), y = count)) + geom_bar(stat = "identity") + geom_text(aes(x = reorder(poster, -count), y = count, label = count, group = status), position = position_dodge(width = 1), vjust = -0.5)
posterPlot
